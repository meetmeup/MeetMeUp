...
- (void)drawRect:(CGRect)rect
{
    ...
    NSString *upperCase = [[formatter stringFromDate:self.currentMonth] uppercaseString];
    
    //add space
    NSMutableString *stringFormatted = [NSMutableString stringWithFormat:@"%@",upperCase];
    for(NSInteger i=[stringFormatted length]-1;i>0;i=i-1) {
        if (i > 0)
        {
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithRed:249.0f/255.0f green:103.0f/255.0f blue:30.0f/255.0f alpha:1.0f].CGColor);
    CGContextFillPath(context);
    
    //Arrows
    int arrowSize = 12;
    int xmargin = 20;
    int ymargin = CURRENT_MONTH_LABEL_Y + 2;
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        UIFont *font = [UIFont fontWithName:@"HelveticaNeue-Light" size:9.63];
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), CURRENT_MONTH_LABEL_Y + 30, kVRGCalendarViewDayWidth+2, 20) withFont:font lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextAddRect(context, rectangleGrid);
            CGContextSetFillColorWithColor(context, [UIColor colorWithRed:249.0f/255.0f green:103.0f/255.0f blue:30.0f/255.0f alpha:0.70f].CGColor);
            CGContextFillPath(context);
        ...
        [date drawInRect:CGRectMake(targetX+1, targetY+14, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:15] lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:16];
